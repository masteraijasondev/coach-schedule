-- Owner assigns which work types each staff member may report after a shift.

create table public.staff_work_types (
...
alter table public.staff_work_types enable row level security;

create policy "staff_work_types_select_own_or_employer"
  on public.staff_work_types for select
  using (coach_id = auth.uid() or public.is_employer());

create policy "staff_work_types_employer_write"
  on public.staff_work_types for all
  using (public.is_employer())
  with check (public.is_employer());

grant select, insert, delete on public.staff_work_types to authenticated;

drop function if exists public.confirm_staff_lesson_periods(uuid, jsonb);

create function public.confirm_staff_lesson_periods(
  p_id uuid,
  p_periods jsonb,
...
revoke all on function public.confirm_staff_lesson_periods(uuid, jsonb, uuid) from public;
grant execute on function public.confirm_staff_lesson_periods(uuid, jsonb, uuid)
  to authenticated;
