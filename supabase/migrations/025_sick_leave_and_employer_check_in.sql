-- Sick leave on an assigned shift, and employers may edit a staff check-in.

alter table public.staff_leaves
  add column if not exists kind text not null default 'leave';

alter table public.staff_leaves
  drop constraint if exists staff_leaves_kind;

alter table public.staff_leaves
  add constraint staff_leaves_kind check (kind in ('leave', 'sick'));
...
begin
  ...
  if v_was_assigned then
    ...
    loop
      v_start := (v_period->>'start_minute')::integer;
      v_end := (v_period->>'end_minute')::integer;
      if v_start > v_cursor then
        v_starts_at := (
          v_date::timestamp + make_interval(mins => v_cursor)
        ) at time zone 'Asia/Hong_Kong';
        v_ends_at := (
          v_date::timestamp + make_interval(mins => v_start)
        ) at time zone 'Asia/Hong_Kong';
    ...
    if v_cursor < v_assign_end then
      v_starts_at := (
        v_date::timestamp + make_interval(mins => v_cursor)
      ) at time zone 'Asia/Hong_Kong';
      v_ends_at := (
        v_date::timestamp + make_interval(mins => v_assign_end)
      ) at time zone 'Asia/Hong_Kong';
      insert into public.lessons (
        lesson_type_id,
        starts_at,
